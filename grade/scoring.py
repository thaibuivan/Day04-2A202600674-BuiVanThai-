...
def main() -> int:
    ...
    parser.add_argument("--provider", default="google", choices=["google", "ollama", "groq", "fireworks"])
    parser.add_argument("--model-name", default=None)
    parser.add_argument("--today", default="2026-06-01")
    parser.add_argument("--pass-threshold", type=float, default=80.0)
    parser.add_argument("--judge-provider", default=None, choices=["google", "ollama", "groq", "fireworks"])
    parser.add_argument("--judge-model-name", default=None)
    parser.add_argument("--delay", type=float, default=7.0, help="Seconds to wait between test cases (default: 7s for free-tier rate limits).")
    parser.add_argument("--max-cases", type=int, default=None, help="Maximum number of cases to run (useful to save quota).")
    ...
    for i, case in enumerate(cases):
        if i > 0 and args.delay > 0:
            print(f"[{i}/{len(cases)}] Waiting {args.delay}s before next case...", flush=True)
            time.sleep(args.delay)
        print(f"[{i+1}/{len(cases)}] Running case: {case['id']}", flush=True)
